...
- `GET /api/export/orders` — auth via a static API key in a request header (`x-api-key`, stored in env). Read-only. Sits **outside** the user-session middleware (excluded by `middleware.ts`).
