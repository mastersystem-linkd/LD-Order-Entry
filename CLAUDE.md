...
- **Order Status** module (`components/order-status/*`, `lib/order-status.ts`, `app/api/order-status`): read-only board grouped **by order** (client-side `aggregateOrderGroups`); expandable rows; cancelled child rows struck, fully-cancelled parents tagged "Cancelled"; a **column picker** (`column-picker.tsx`, `useColumnPrefs`, per-user localStorage). KPI cards filter in place (Total / In progress / Completed / Overdue / **Cancelled = orders with a cancellation**); initial filter can be **deep-linked** via `?overall=` / `?cancelled=1` / `?stage=` (page wraps the board in `<Suspense>` for `useSearchParams`). A fully-cancelled group's `currentStageKey` is `null` so it never leaks into the "At stage" filter. Detail drawer `status-drawer.tsx` + CSV export stay line-level; deleted lines excluded, cancelled shown.
