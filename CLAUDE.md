...
3. **Operations tracking is per LINE ITEM** through 7 stages — `order_entry`, `stock_checking`, `rolling_checking`, `challan`, `bill`, `dispatch`, `received_lr` — each with planned/actual datetime, done flag, delay (minutes). Gating is **stock-only** (§6): `order_entry` and `stock_checking` are always editable; `stock_checking` is a 3-way gate (**Pending / In stock / Out of stock**) and only **In stock** unlocks the five stages after it — which then complete in **any order**. Un-ticking is always allowed (no strict sequence, no downgrade block).
...
- **Stage completion — STOCK-ONLY gating** (all in `lib/workflow.ts`, ONE transaction; a rule violation throws `WorkflowError` → the stage API returns **409** with the message):
  - `order_entry` and `stock_checking` have **no prerequisite**. The five stages **after** `stock_checking` can be completed **only once stock is `in_stock`** — then in **any order** (no strict sequence; e.g. challan without rolling).
